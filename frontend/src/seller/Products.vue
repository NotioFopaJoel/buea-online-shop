<template>
  <div class="container-bos py-6">
    ...
    <div class="flex items-center justify-between mb-4">
      <h1 class="font-display font-bold text-2xl">My Products</h1>
    ...
    <div v-else class="rounded-card card-surface overflow-hidden">
      ...
      <p v-if="!products.length" class="text-sm text-center py-8" style="color: var(--text-secondary);">You haven't added any products yet.</p>
    </div>

    <Modal v-model="showModal" :title="editing?._id ? 'Edit Product' : 'Add Product'">
...
async function fetchAll() {
  ...
  try {
    const [prodRes, catRes] = await Promise.all([
      api.get('/products', { params: { sellerId: authStore.user._id, limit: 100 } }),
      api.get('/categories'),
    ]);
...
async function handleSave() {
  ...
  try {
    if (editing.value) {
      await api.put(`/products/${editing.value._id}`, form);
    } else {
